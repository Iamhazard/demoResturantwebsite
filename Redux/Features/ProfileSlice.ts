...
const API_URL = `${BACKEND_URL}/auth`;
    ...
    '/profile',
    async (credentials: {formData:ProfileFormValues}, thunkAPI) => {
        try {
            const response = await axios.post(`${API_URL}/profile`, credentials);
            const profile=response.data.profile;
            LoginSchema.parse(profile)
    ...
    extraReducers:(builder)=>{
        builder.addCase(createProfile.pending,(state,action)=>{
         ...
         .addCase(createProfile.fulfilled, (state, action: PayloadAction<{
             user: User; profile: ProfileFormValues 
}>) => {
            ...
            .addCase(editProfiel.fulfilled, (state, action: PayloadAction<{ user: Users ,profile:ProfileFormValues}>) => {
                state.status = 'succeeded';
                state.profile = action.payload.profile;
                state.isAdmin = action.payload.user.role === 'ADMIN';
            ...
            .addCase(fetchCurrentProfile.fulfilled, (state, action: PayloadAction<{ user: Users ,profile:ProfileFormValues}>) => {
                state.status = 'succeeded';
                state.profile = action.payload.profile;
                state.isAdmin = action.payload.user.role === 'ADMIN';
